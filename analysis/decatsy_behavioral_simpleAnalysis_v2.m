...
function []=decatsy_behavioral_simpleAnalysis_v2(subject_ind, arg2,...
    ...
    if allblocksEEGsess % && ~exist(torejtrialsbehav_file)
        ...
        save(sprintf('./decatsy_data/subj%i/log_files/subj%i_sess%i_behav_trials_to_rej.mat',...
            subject_ind,subject_ind,session), 'rej_behav_trials');
    end
    ...
    % reject trials that were rejected by visual inspection of the EEG
    if allblocksEEGsess && rejectEEGrejectedTrials
        eeg_trials_torej_file=sprintf('%ss%i_sess%i_eeg_trials_to_rej.mat',...
            subj_eeg_dir, s_ind, session);
        if exist([subj_eeg_dir eeg_trials_torej_file], 'file')
            % load the manually rejected trials
            load([subj_eeg_dir eeg_trials_torej_file]);
            maskrejeeg=logical(ones(length(correctResp),1)); maskrejeeg(eegtrialrej)=0;
            
            correctResp=correctResp(maskrejeeg); cue=cue(maskrejeeg);
            precue=precue(maskrejeeg); respTime=respTime(maskrejeeg);
            gratingOriR=gratingOriR(maskrejeeg); validity=validity(maskrejeeg);
            correctSide=correctSide(maskrejeeg); correctTilt=correctTilt(maskrejeeg);
            tiltsLvlH=tiltsLvlH(maskrejeeg); tiltsLvlV=tiltsLvlV(maskrejeeg);
            triali=triali(maskrejeeg); respKey=respKey(maskrejeeg);
            gratingOriL=gratingOriL(maskrejeeg); block=block(maskrejeeg);
            % SAVE ALL THESE VARIABLE FOR FURTHER USE
            save([study_dir sprintf('log_files/subj%i_sess%i_log_consistentEEG.mat',subject_ind,session)],...
            'correctResp', 'cue', 'precue', 'respTime', 'gratingOriR', 'validity', 'correctSide', 'correctTilt', 'tiltsLvlH', 'tiltsLvlV', 'triali', 'respKey', 'gratingOriL', 'block', 'maskrejeeg')
        else
            fprintf('THERE WAS NO FILES CONTAINING EEG TRIALS REJECTED!!');
            return;
        end
    ...
    switch experiment_phase
        case {'train1', 'train2', 'train4', 'main'}
            ...
            xlabetxt = {'Hori Valid Left' 'Hori Invalid Left'...
                ...
                'Vert Invalid Left' 'Vert Valid Right' 'Vert Invalid Right'};
            ypos = min(ylim)*.99;
